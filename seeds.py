...
db = Session() # Anytime we want to perform a CRUD operation using SQLAlchemy, we need to establish a temporary session connection with the

# insert users
db.add_all([    # preps the SQL queries
...
db.commit() # To run the INSERT statements, you need to call db.commit()

# insert posts - run this add_all() method after creating the users, because the Post model requires valid user_id fields.
db.add_all([
...
db.commit()  # db.commit() again to confirm these latest insertions in the database.
...
db.close()  # If you don't need to make any other database transactions at this time, you can close the session connection by calling